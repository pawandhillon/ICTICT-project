...
Plugin URI: http://webcarezone.com/projects/sb-login.asp
Description: Sb login widget that allows a user to login, register, reset their password, see recent activity,time , post and comment count ALL without leaving their current location! It also allow to change wordpress email form name and address.
Version: 2.5
...
load_plugin_textdomain('sb-login', $nd_login_vars['plugin_url'] . '/langs/', 'sb-login/langs/');

###[ Detect Ajax ]###############################################[ WEBCAREZONE.COM ]####
...
###[ Get Current URL ]###########################################[ WEBCAREZONE.COM ]####
...
###[ Update user data upon logging in ]###########################################[ WEBCAREZONE.COM ]####
...
###[ Update user data upon viewing post ]###########################################[ WEBCAREZONE.COM ]####
...
function nd_login_init() {
	class NinetyAjaxLogin extends WP_Widget {
	    function NinetyAjaxLogin() {  
	        $widget_ops = array('description' => __( 'An Ajax powered Login &amp; Register widget. See the ReadMe for customisation instructions.','sb-login') );
			$this->WP_Widget('nd_ajax_login', __('SB Login','sb-login'), $widget_ops);  
	    }
...
###[ Load Templates ]###############################################[ WEBCAREZONE.COM ]####
...
###[ Proces Login/Register ]###################################################[ WEBCAREZONE.COM ]####
...
function nd_handle_login() {
	...
	if ( !$_POST['log'] ) :
		$user = new WP_Error();
		$user->add('empty_username', __('<strong>ERROR</strong>: Please enter a username.', 'sb-login'));
	elseif ( !$_POST['pwd'] ) :
		$user = new WP_Error();
		$user->add('empty_username', __('<strong>ERROR</strong>: Please enter your password.', 'sb-login'));
	endif;
...
function nd_handle_register() {
	...
	// Check the username
	if ( $posted['username'] == '' )
		$errors->add('empty_username', __('<strong>ERROR</strong>: Please enter a username.', 'sb-login'));
	elseif ( !validate_username( $posted['username'] ) )
		$errors->add('invalid_username', __('<strong>ERROR</strong>: This username is invalid.  Please enter a valid username.', 'sb-login'));
	elseif ( username_exists( $posted['username'] ) )
		$errors->add('username_exists', __('<strong>ERROR</strong>: This username is already registered, please choose another one.', 'sb-login'));
		
	// Check the e-mail address
	if ($posted['email'] == '')
		$errors->add('empty_email', __('<strong>ERROR</strong>: Please type your e-mail address.', 'sb-login'));
	elseif ( !is_email( $posted['email'] ) )
		$errors->add('invalid_email', __('<strong>ERROR</strong>: The email address isn&#8217;t correct.', 'sb-login'));
	elseif ( email_exists( $posted['email'] ) )
		$errors->add('email_exists', __('<strong>ERROR</strong>: This email is already registered, please choose another one.', 'sb-login'));
			
	// Check Passwords match
	if ($posted['password'] == '')
		$errors->add('empty_password', __('<strong>ERROR</strong>: Please enter a password.', 'sb-login'));
	elseif ($posted['password'] !== $posted['password2'])
		$errors->add('wrong_password', __('<strong>ERROR</strong>: Passwords do not match.', 'sb-login'));
	else
	...
	if ( !$errors->get_error_code() ) :
		...
		if ( !$user_id ) :
			$errors->add('registerfail', sprintf(__('<strong>ERROR</strong>: Couldn&#8217;t register you... please contact with the site admin !', 'sb-login'), get_option('admin_email')));
		else :
...
function nd_handle_lost_password() {
	...
	if ( empty( $_POST['username_or_email'] ) ) $errors->add('empty_username', __('<strong>ERROR</strong>: Enter a username or e-mail address.', 'sb-login'));

	if ( strpos($_POST['username_or_email'], '@') ) {
		$user_data = get_user_by_email(trim($_POST['username_or_email']));
		if ( empty($user_data) ) $errors->add('invalid_email', __('<strong>ERROR</strong>: There is no user registered with that email address.', 'sb-login'));
	} else {
	...
	if ( !$user_data ) $errors->add('invalidcombo', __('<strong>ERROR</strong>: Invalid username or e-mail.', 'sb-login'));
	...
	if ( !$allow ) $errors->add('no_password_reset', __('Password reset is not allowed for this user', 'sb-login'));
	...
	$message = __('Someone has asked to reset the password for the following site and username.', 'sb-login') . "\r\n\r\n";
	$message .= network_site_url() . "\r\n\r\n";
	$message .= sprintf(__('Username: %s', 'sb-login'), $user_login) . "\r\n\r\n";
	$message .= __('To reset your password visit the following address, otherwise just ignore this email and nothing will happen.', 'sb-login') . "\r\n\r\n";
	...
	$title = sprintf( __('[%s] Password Reset', 'sb-login'), $blogname );
	...
	if (nd_is_ajax()) :
		echo 'SUCCESS:'.__('Check your e-mail for the confirmation link.', 'sb-login');
		exit;
	endif;
